// ==UserScript==
// @name         ChatHacks (wikia chat)
// @namespace    http://dev.wikia.com/wiki/MediaWiki:ChatHacks.js
// @version      7.0
// @description  See the features below
// @author       Monchoman45
// @match        none
// @grant        none
// ==/UserScript==
...
if (wgCanonicalSpecialPageName == 'Chat') {
		...
		for(var i in this.model.users.models) {
			if(this.model.users.models[i].attributes.name == chat.attributes.name) {
				if(this.model.users.models[i].attributes.isStaff) {
					icon = ' <img class="stafficon" src="http://vignette2.wikia.nocookie.net/wikiaglobal/images/6/6c/Staff_icon.png/revision/latest?cb=20160714211241">';
				}
	...
	// return true to send text to chat unparsed
	//        false to discard it after parsing
		...
		if ((input.charAt(0) === '/') && (ref.length > 0)) {
			while (!done) {
				...
				case 'string':
					...
					break;
				// parse hierarchical commands, if ever there are any
				case 'object':
					commands = commands[ref];
					if (com.length === 0) {
						// there is no next keyword
						ref = 'fallback';
					} else {
						ref = com[0];
						if (typeof commands[ref] === 'undefined') {
							// the next keyword isn't a command, so it's an argument
							ref = 'fallback';
						} else {
							// the next keyword is a command, so one less argument
							com.shift();
						}
					}
					break;
				// it's a function or it's not; either way, we're done
				default:
					done = true;
				}
			}
			if (typeof commands[ref] === 'function') {
				return commands[ref].call(this, com.join(' '), input);
			}
		}
		return true;
	}
	...
	window.commands = {
		...
		'unban': function (com, text) { //Unban a user
			...
			mainRoom.socket.send(new models.BanCommand({
				userToBan: com // Empty ban command === unban
			}).xport());
		}
		else {this.inlineAlert(i18n['erruser'].replace(/\$1/g, text.split(' ')[0]));}
		},
		...
		'demod': function (com, text) { //Remove a user's mod right
			if(com) {
			$.getJSON(mw.util.wikiScript('api') + '?action=query&list=users&ustoken=userrights&ususers=' + com + '&format=json', function (token) {
				$.post(mw.util.wikiScript('api'), {
					format: 'json',
					action: 'userrights',
					user: com,
					remove: 'chatmoderator',
					...
					if(result.userrights.removed[0] == 'chatmoderator') {
						...
						this.inlineAlert(i18n['notmod'].replace(/\$1/g, result.userrights.user));
					}
				});
			});
		}
		...
		'private': function (com, text) { //Invoke a private room
			if (com.length > 0) {
				...
				var curRoom = mainRoom.model.privateUsers.findByName(users[0]);
				if (!curRoom) {
					mainRoom.openPrivateChat(users);
				} else {
					mainRoom.showRoom(curRoom.attributes.roomId);
				}
			} else {
				this.inlineAlert(i18n.erruser.replace(/\$1/g, text.replace(/\s+$/, '')));
			}
		},
		/*'enter': 'join',
